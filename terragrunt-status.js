...
const outputStream = new Writable({ write(chunk, encoding, callback) { if (argv.debug) process.stderr.write(chunk); callback(); } })
...
async function getDependencyTree(spinnies) {
  ...
  try {
    ...
    const onlyEdges = [...new Set(parsedGraph[0].children.filter(obj => obj.type === "edge_stmt").map(obj => obj.edge_list).map(arr => arr.map(ids => ids.id)))];
    const destroyOrder = toposort(onlyEdges)
    const otherNodes = parsedGraph[0].children.filter(obj => obj.type === "node_stmt").map(obj => obj.node_id.id).filter(obj => !destroyOrder.includes(obj))
    const returnVal = {
      destroyOrder: [...destroyOrder],
      deployOrder: [...destroyOrder].reverse(),
      noDeps: otherNodes
    }
    spinnies.succeed("deptree", { text: `Getting dependency tree... Done! (${destroyOrder.length + otherNodes.length} stacks)` })
    return returnVal;
  } catch (e) {
...
async function getStatusAll(dependencyTree, spinnies) {
  let results = await Promise.all(dependencyTree.deployOrder.map(stack => {
    let dir = path.resolve(cwd, stack);
    spinnies.add(dir, { text: chalk`Checking if {blue ${stack}} is deployed...` })
    return getStatus(dir, spinnies, stack)
  }))
  results.forEach(result => {
    if (result.stacktrace) outputStream.write(result.stacktrace);
  })
}
...
async function isDeployed(dir, spinnies, stackName) {
  ...
  try {
    ...
    if (ret.deployed) {
      ...
      spinnies.fail(dir, { text: chalk`{blue ${stackName}} is NOT deployed! ${ret.failReason ? `(Reason: ${ret.failReason})` : ""} ${ret.failReason ? chalk`{grey (use --debug to show errors from Terragrunt)}` : ""}` })
    }
...
async function getPlan(dir, spinnies, stackName) {
  ...
  if (!argv.r && !argv.refresh) tgPlanArgs.push('-refresh=false', '-lock=false');
  ...
  try {
    ...
    if (ret.hasChanges) {
      ...
      spinnies.succeed(dir, { text: chalk`{blue ${stackName}} is up to date!` })
    } else {
      spinnies.fail(dir, { text: chalk`{blue ${stackName}} errored :( ${chalk`{grey (use --debug to show errors from Terragrunt)`}` })
    }
